...
function VertexBufferReader(geometry) {
	...
	this.texData =
		this.useCompactBuffers &&
		geometry.tIdColor?.image?.data &&
		new Uint32Array(geometry.tIdColor.image.data.buffer);
...
VertexBufferReader.prototype.decodeLineAt = function (vindex, layer, vpId, callback) {
	...
	if (this.useCompactBuffers) {
		...
		var y0 =
			(this.useCompactBuffers.y * this.vbs[vertexOffset + 1]) / 65535 + this.useCompactBuffers.w;
		var angle = (this.vbs[vertexOffset + 2] / 65535) * TAU - Math.PI;
		var distAlong =
			(this.vbs[vertexOffset + 3] / 65535) *
			Math.max(this.useCompactBuffers.x, this.useCompactBuffers.y);
		var lineWidth =
			(this.vbs[vertexOffset + 4] / 32767) *
			Math.max(this.useCompactBuffers.x, this.useCompactBuffers.y) *
			2.0;
	} else {
	...
	callback.onLineSegment(x0, y0, x1, y1, vpId, lineWidth, vindex);
};
...
VertexBufferReader.prototype.decodeCircularArcAt = function (vindex, layer, vpId, callback) {
	...
	callback.onCircularArc(cx, cy, start, end, radius, vpId, vindex);
};
...
VertexBufferReader.prototype.decodeEllipticalArcAt = function (vindex, layer, vpId, callback) {
	...
	callback.onEllipticalArc(cx, cy, start, end, major, minor, tilt, vpId, vindex);
};
...
VertexBufferReader.prototype.decodeTexQuadAt = function (vindex, layer, vpId, callback) {
	...
	if (this.useCompactBuffers) {
		var vertexOffset = this.stride * vindex * 2;
		var centerX =
			(this.useCompactBuffers.x * this.vbs[vertexOffset]) / 65535 + this.useCompactBuffers.z;
		var centerY =
			(this.useCompactBuffers.y * this.vbs[vertexOffset + 1]) / 65535 + this.useCompactBuffers.w;
	...
	callback.onTexQuad(centerX, centerY, width, height, rotation, vpId, vindex);
};
...
VertexBufferReader.prototype.decodeOneTriangleAt = function (vindex, layer, vpId, callback) {
	...
	if (this.useCompactBuffers) {
		...
		var y1 =
			(this.useCompactBuffers.y * this.vbs[vertexOffset + 1]) / 65535 + this.useCompactBuffers.w;
		var x2 =
			(this.useCompactBuffers.x * this.vbs[vertexOffset + 2]) / 65535 + this.useCompactBuffers.z;
		var y2 =
			(this.useCompactBuffers.y * this.vbs[vertexOffset + 3]) / 65535 + this.useCompactBuffers.w;
		var x3 =
			(this.useCompactBuffers.x * this.vbs[vertexOffset + 4]) / 65535 + this.useCompactBuffers.z;
		var y3 =
			(this.useCompactBuffers.y * this.vbs[vertexOffset + 5]) / 65535 + this.useCompactBuffers.w;
	} else {
		var baseOffset = this.stride * vindex;
	...
	callback.onOneTriangle(x1, y1, x2, y2, x3, y3, vpId, vindex);
};

VertexBufferReader.prototype.decodeTriangleIndexed = function (
	vi0,
	vi1,
	vi2,
	layer,
	...
	callback.onOneTriangle(x1, y1, x2, y2, x3, y3, vpId, vi0, vi1, vi2);
};
