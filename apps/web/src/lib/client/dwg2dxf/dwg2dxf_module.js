import dwgWasm from './dwg2dxf_module.wasm?url';
...
export default function (createMyModule) {
	...
	var INITIAL_MEMORY = Module['INITIAL_MEMORY'] || 1024 * 1024 * 64;
	var wasmTable;
	var __ATPRERUN__ = [];
	var __ATINIT__ = [];
	var __ATMAIN__ = [];
	...
	async function getBinaryPromise() {
		return await fetch(dwgWasm).then((response) => response.arrayBuffer());
	}
	function createWasm() {
		...
		function receiveInstance(instance, module) {
			...
			var TOTAL_MEMORY = 1024 * 1024 * 64;
			var WASM_PAGE_SIZE = 65536;
			var wasmMemory = new WebAssembly.Memory({
				initial: TOTAL_MEMORY / WASM_PAGE_SIZE,
				maximum: TOTAL_MEMORY / WASM_PAGE_SIZE
			});
			// module['asm']['memory'] = wasmMemory;
		...
		function instantiateArrayBuffer(receiver) {
			return getBinaryPromise()
				.then(function (binary) {
					let inst = new WebAssembly.Instance(binary, info);
		...
		function instantiateAsync() {
			if (typeof WebAssembly.instantiateStreaming == 'function' && typeof fetch == 'function') {
				return fetch(dwgWasm, { credentials: 'same-origin' }).then(function (response) {
	...
	function abortOnCannotGrowMemory(requestedSize) {
		abort('OOM ' + (requestedSize / (1024 * 1024)).toFixed(2) + 'mb');
	}
