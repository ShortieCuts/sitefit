...
export class SuperZoomMapType implements google.maps.ImageMapType {
		...
		for (let i: number = 0; i < tilesLength; i++) {
			...
			if (
				((minX < maxX && tileX >= minX && tileX <= maxX) ||
					(minX > maxX &&
						((tileX >= minX && tileX <= Math.pow(2, zoom) - 1) ||
							(tileX >= 0 && tileX <= maxX)))) && // Lapped the earth!
				tileY >= minY &&
				tileY <= maxY &&
				tileZ === zoom
			) {
	...
	public pointToTile(latlng: google.maps.LatLng, z: number): google.maps.Point {
		...
		let pixelCoordinate: google.maps.Point = new google.maps.Point(
