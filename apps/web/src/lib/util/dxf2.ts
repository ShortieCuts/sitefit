...
function dwg_model_x_min(dwg: IDxf) {
	return dwg.header.$EXTMIN.x;
}

function dwg_model_y_min(dwg: IDxf) {
	return dwg.header.$EXTMIN.y;
}

function dwg_model_x_max(dwg: IDxf) {
	return dwg.header.$EXTMAX.x;
}

function dwg_model_y_max(dwg: IDxf) {
	return dwg.header.$EXTMAX.y;
}

function dwg_page_x_min(dwg: IDxf) {
	return dwg.header.$PEXTMIN.x;
}

function dwg_page_y_min(dwg: IDxf) {
	return dwg.header.$PEXTMIN.y;
}

function dwg_page_x_max(dwg: IDxf) {
	return dwg.header.$PEXTMAX.x;
}

function dwg_page_y_max(dwg: IDxf) {
	return dwg.header.$PEXTMAX.y;
}
...
export function translateDXF(rawDXF: string): Object2D[] | null {
	...
	function transform_X(x: number) {
		return x - model_xmin;
	}

	function transform_Y(y: number) {
	...
	try {
		...
		// model_xmin = dwg_model_x_min(dxf);
		// model_ymin = dwg_model_y_min(dxf);

		// double dx = (dwg_model_x_max(dxf) - dwg_model_x_min(dxf));
		// double dy = (dwg_model_y_max(dxf) - dwg_model_y_min(dxf));
		// double scale_x = dx / (dwg_page_x_max(dxf) - dwg_page_x_min(dxf));
		// double scale_y = dy / (dwg_page_y_max(dxf) - dwg_page_y_min(dxf));
		// //scale = 25.4 / 72; // pt:mm
		// page_width = dx;
		// page_height = dy;
